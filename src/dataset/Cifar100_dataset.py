...
data_pattern = os.path.join("./data/cifar_splits", pattern) # path for dataloaders 
...
for i in range(num_tasks):
	...
	if not os.path.isdir(base_path):
		...
		for j in range(num_classes):
			os.makedirs(base_path+"/train/"+str(j))
			os.makedirs(base_path+"/test/"+str(j))
...
testset = torchvision.datasets.CIFAR100(root="./data", train=False, download=True, transform=transform_tensor)
